...
d2$u5_oORF <- unlist(lapply(1:nrow(d2), function(i) length(get_oorf(d1$five_prime_UTR[i], d1$CDS[i], inframe = F, share_stops = F))))
d2$u5_oORF_total <- unlist(lapply(1:nrow(d2), function(i) length(get_oorf(d1$five_prime_UTR[i], d1$CDS[i], share_stops = F))))

d2$u5_NTE <- unlist(lapply(1:nrow(d2), function(i) length(get_oorf(d1$five_prime_UTR[i], d1$CDS[i], inframe = T, share_stops = F))))
...
d2 <- d2[!duplicated(d2),]
fwrite(d2, 'derived/tables/210709_MANE.v0.95.UTR_features.txt', sep = '\t')
