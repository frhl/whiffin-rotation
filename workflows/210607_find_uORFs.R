...
d2$u5_oORF <-  unlist(lapply(1:nrow(d2), function(i) length(get_oorf(d1$five_prime_UTR[i], d1$CDS[i]))))
...
d2$u3_oORF <-  unlist(lapply(1:nrow(d2), function(i) length(get_oorf(d1$CDS[i], d1$three_prime_UTR[i]))))
d2$u3_max_kozak <- unlist(lapply(d1$three_prime_UTR, function(x) max(unlist(get_utr_kozak_strength(x)))))


d2$u5_ORF <- unlist(lapply(1:nrow(d1), function(i) {
  
  print(i)
  #lens <- length(find_orfs(d1$five_prime_UTR[i]))
  max(unlist(get_utr_kozak_strength(d1$five_prime_UTR[i])))
...
# look at GC content
plot(density(na.omit(gc_content[d$type == 'CDS'])), main = 'GC content (%)', xlim = c(0.2,1), ylim = c(0, 4))
