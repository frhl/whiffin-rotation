...
# Load three prime data
d3_obs_ci <- fread('derived/210706_MANE.GRCh38.v0.95_three_prime_utr_codons_expt_ci.csv', sep = ',')
d3_obs <- fread('derived/210706_MANE.GRCh38.v0.95_three_prime_utr_codons_expt.csv', sep = ',')
d3_expt <- fread('derived/210706_MANE.GRCh38.v0.95_three_prime_utr_codons_obs.csv', sep = ',')

# merge observed / expected
mrg3 <- merge(d3_expt, d3_obs)
obs3 <- mrg3[,get('obs',mrg3), with = F] 
expt3 <- mrg3[,get('expt',mrg3), with = F] 
lens3 <- rowSums(obs3)
d3 <- as.data.frame(colSums(obs3) / colSums(expt3))
colnames(d3) <- 'oe'
d3$codon <- unlist(lapply(strsplit(rownames(d3), split = '\\.'), function(x) x[2]))
d3$utr <- 3

# Load five prime data
d5_obs_ci <- fread('derived/210701_MANE.GRCh38.v0.95_codons_expt_ci.csv', sep = ',')
d5_obs <- fread('derived/210701_MANE.GRCh38.v0.95_codons_expt.csv', sep = ',')
d5_expt <- fread('derived/210701_MANE.GRCh38.v0.95_codons_obs.csv', sep = ',')

# merge observed / expected
mrg5 <- merge(d5_expt, d5_obs)
obs5 <- mrg5[,get('obs',mrg5), with = F] 
expt5 <- mrg5[,get('expt',mrg5), with = F] 
lens5 <- rowSums(obs5)
d5 <- as.data.frame(colSums(obs5) / colSums(expt5))
colnames(d5) <- 'oe'
d5$codon <- unlist(lapply(strsplit(rownames(d5), split = '\\.'), function(x) x[2]))
d5$utr <- 5

# plot data
d <- rbind(d5, d3)
d$utr <- as.factor(d$utr)
ggplot(d, aes(x=reorder(codon, oe), y = oe, color =  utr)) +
  geom_point() +
